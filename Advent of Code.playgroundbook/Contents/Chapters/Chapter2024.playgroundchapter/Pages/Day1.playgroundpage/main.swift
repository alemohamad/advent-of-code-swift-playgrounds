...
calculateTotalDistance(left: leftList, right: rightList)
 ...
 * Callout(Here are the same example lists again:):
   `3   4`  
   `4   3`  
   `2   5`  
   `1   3`  
...
func checkSolution() {
    ...
    if resultSimilarity == expectedResultSimilarity {
        ...
        print("❌ Test Failed. Expected \(expectedResultSimilarity), but got \(resultSimilarity).")
    }
}
