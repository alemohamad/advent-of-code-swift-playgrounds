...
func countSafeReports(input: String) -> Int {
    ...
    return reports.filter { isReportSafe(levels: $0) }.count
}
...
func isReportSafeWithDampener(levels: [Int]) -> Bool {
    if isReportSafe(levels: levels) { return true }
    
    for i in 0..<levels.count {
        var modifiedLevels = levels
        modifiedLevels.remove(at: i)
        if isReportSafe(levels: modifiedLevels) {
            return true
        }
    }
...
func countSafeReportsWithDampener(input: String) -> Int {
    ...
    return reports.filter { isReportSafeWithDampener(levels: $0) }.count
}
...
"""
    ...
    let expectedSafeReports = 2
    let resultSafeReports = countSafeReports(input: exampleInput)
    if resultSafeReports == expectedSafeReports {
    ...
    if resultSafeReportsWithDampener == expectedSafeReportsWithDampener {
        print("✅ Test Passed! Your solution is correct.")
    } else {
        print("❌ Test Failed. Expected \(expectedSafeReportsWithDampener), but got \(resultSafeReportsWithDampener).")
    }
}
